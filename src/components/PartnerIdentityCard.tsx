...
import { cn, getBase64ImageFromURL } from "@/lib/utils";
...
export const PartnerIdentityCard = () => {
  ...
  // Card dimensions for ID card standard (CR80)
  const CARD_WIDTH = 85.6; // mm
  const CARD_HEIGHT = 54; // mm
  const SCALE_FACTOR = 4; // Higher scale for better quality

  useEffect(() => {
    if (userData?.photoUrl) {
      getBase64ImageFromURL(userData.photoUrl)
        .then(setPhotoBase64)
        .catch(() => setPhotoBase64("/default-user.png"));
    }
  }, [userData]);
...
await new Promise((r) => setTimeout(r, 1000));
      ...
      try {
        ...
        if ((options.frontOnly || options.complete) && !frontRef.current) {
          throw new Error("Front card element not found");
        }
        if ((options.backOnly || options.complete) && !backRef.current) {
          throw new Error("Back card element not found");
        }
...
await Promise.all(
    ...
    (img: any) =>
      new Promise((resolve) => {
        if (img.complete) resolve(true);
          ...
          pdf.addImage(imgDataFront, "PNG", 0, 0, CARD_WIDTH, CARD_HEIGHT);
        }

        // Generate back side
        if (options.backOnly) {
          const canvasBack = await html2canvas(backRef.current!, canvasOptions);
          const imgDataBack = canvasBack.toDataURL("image/png", 1.0);
          pdf.addImage(imgDataBack, "PNG", 0, 0, CARD_WIDTH, CARD_HEIGHT);
        ...
        const fileName = options.frontOnly
          ? `Amaze_ID_Front_${userData?.username || "user"}.pdf`
          : options.backOnly
            ? `Amaze_ID_Back_${userData?.username || "user"}.pdf`
            : `Amaze_ID_Card_${userData?.username || "user"}.pdf`;
  ...
  const renderCardFront = () => (
    <div
      ref={frontRef}
      style={{
        width: `${CARD_WIDTH * 3.78}px`, // Convert mm to px (1mm ≈ 3.78px at 96dpi)
        height: `${CARD_HEIGHT * 3.78}px`,
      }}
      className="rounded-[16px] flex overflow-hidden shadow-2xl bg-gradient-to-br from-slate-800 to-slate-900"
    >
      {/* Left Panel - Brand */}
      <div className="w-[32%] bg-gradient-to-b from-emerald-600 to-emerald-800 flex flex-col items-center justify-center p-3 relative">
        ...
        <div className="relative z-10 flex flex-col items-center gap-1.5">
          <div className="relative">
            <img src="/amaze-logo.png" width={48} height={48} />
          ...
          <p className="text-[7px] font-bold text-emerald-200 uppercase tracking-widest">
            Ayurveda
          </p>
        </div>
    ...
    >
      ...
      <div className="flex justify-between items-start relative z-10 flex-1">
        ...
        <div className="flex flex-col justify-between h-full py-1 pr-3">
          <div className="space-y-2.5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[7px] font-black text-slate-700 uppercase tracking-wider">
                Address
              </p>
            </div>
          ...
          <div className="space-y-2">
            {userData?.email && (
              <div className="flex items-center gap-1.5 text-slate-500">
                ...
                <span className="text-[6px] font-medium truncate max-w-[100px]">
                  {userData.email}
                </span>
              </div>
            )}
            <p className="text-[5px] text-slate-400 max-w-[120px] leading-tight">
              Property of Amaze Ayurveda Pvt. Ltd. If found, return to nearest
              corporate office.
            </p>
            <div className="flex items-center gap-1.5 bg-emerald-100 w-fit px-2 py-1 rounded-full">
              <ShieldCheck className="w-3 h-3 text-emerald-600" />
              <span className="text-[6px] font-bold text-emerald-700 uppercase tracking-wide">
                Swadeshi Partner
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center justify-center gap-1.5 border-l-2 border-slate-200 pl-3 h-full">
          ...
          <p className="text-[5px] text-slate-400 text-center max-w-[55px]">
            Authentic
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-100 relative z-10">
        <div className="flex items-center gap-2">
          <Image
            ...
            unoptimized
          />
          <span className="text-[5px] font-bold text-slate-400 uppercase tracking-wider">
            Amaze Ayurveda
          </span>
        </div>
        <span className="text-[4px] text-slate-400">
          CIN: U82990BR2023PTC066853
        </span>
      </div>
    </div>
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <p className="text-lg font-semibold text-slate-600">
          Please login to view your ID Card
        </p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      >
        ...
        <p className="text-slate-500 max-w-lg mx-auto">
          Your official Amaze Ayurveda partner identification with QR
          verification for instant authentication
        </p>
      </motion.div> */}

      {/* Hidden cards for PDF generation - always rendered but invisible */}
      <div
