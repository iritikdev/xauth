...
import { getAdminInvestmentList, AdminInvestmentData, activateInvestmentAndSetLockIn } from "@/lib/actions/admin"
...
export default function AdminInvestmentList() {
    ...
    // कॉम्पोनेंट के अंदर सबसे ऊपर इस स्टेट को ऐड करें (ताकि लोडिंग के समय बटन्स डिसेबल हो सकें)
    const [processingId, setProcessingId] = useState<string | null>(null)

    const handleAction = async (id: string, action: "APPROVE" | "REJECT") => {
        // Loading स्टेट सेट करें
        setProcessingId(id)

        try {
            if (action === "APPROVE") {
                // 1. अप्रोवल के लिए आपका पुराना बनाया हुआ Server Action कॉल करें
                const response = await activateInvestmentAndSetLockIn(id)

                if (response.success) {
                    toast.success("Investment Approved", {
                        description: "Wallet updated and 30-day lock-in period activated.",
                    })
                } else {
                    toast.error("Approval Failed", { description: "response.error" })
                }
            } else {
                // 2. रिजेक्शन के लिए हैंडलर (अगर आपने अलग से reject action बनाया है, तो यहाँ कॉल करें)
                // उदाहरण के लिए: await rejectInvestment(id)
                toast.error("Investment Rejected", {
                    description: "The investment request has been declined.",
                })
            }

            // डेटा को रीफ्रेश करें ताकि लिस्ट अपडेट हो जाए
            await loadData()
        } catch (error) {
            console.error("Action handler failed:", error)
            toast.error("Execution Error", { description: "Something went wrong processing this request." })
        } finally {
            // Loading स्टेट को हटाएँ
    ...
    return (
        <div className="space-y-4 p-2 sm:p-6 text-zinc-900 max-w-7xl mx-auto">
            {/* Top Bar */}
                        ...
                        <Card key={item.id} className="rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm">
                            <CardContent className="p-0 space-y-3">
                                ...
                                <div className="flex items-start justify-between">
                                    ...
                                    <span className={`rounded-md px-2 py-0.5 text-[10px] font-black uppercase tracking-wider ${item.status === "COMPLETED" ? "bg-emerald-50 text-emerald-700 border border-emerald-200/50" :
                                ...
                                <div className="flex gap-2 pt-1">
                                    ...
                                    {/* {item.status === "PENDING" && (
                    <>
                      <Button onClick={() => handleAction(item.id, "REJECT")} size="sm" variant="outline" className="h-9 rounded-xl border-rose-200 text-rose-600 hover:bg-rose-50 px-3">
                        <X className="h-4 w-4" />
                      </Button>
                      <Button onClick={() => handleAction(item.id, "APPROVE")} size="sm" className="h-9 rounded-xl bg-emerald-500 text-zinc-950 font-bold hover:bg-emerald-400 px-3">
