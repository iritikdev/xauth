...
export default function TransactionHistory({ userId }: TransactionHistoryProps) {
    ...
    const getDaysRemaining = (maturesAtDate: string | null) => {
        if (!maturesAtDate) return null
        const diffTime = new Date(maturesAtDate).getTime() - new Date().getTime()
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))
        return diffDays > 0 ? diffDays : 0
    }

    console.log("Transaction history data:", transactions) // Debug log to verify data structure and content

    // JSX के अंदर रेंडर करें:
                    ...
                    transactions.map((item) => (
                        ...
                        >
                            <div className="flex items-start gap-3 sm:gap-4">
                                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${item.type === "credit" ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"
                                    }`}>
                                    {item.type === "credit" ? (
                                ...
                                <div className="space-y-0.5">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 pt-1">
                                        ...
                                        <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold ${item.status === "VERIFIED"
                                        ...
                                        <span className="text-[11px] font-medium text-zinc-400 font-mono">
                                            {item.maturesAt ? (() => {
                                                const daysLeft = getDaysRemaining(item.maturesAt)
                                                const formattedDate = new Date(item.maturesAt).toLocaleDateString("en-IN", {
                                                    day: "2-digit",
                                                    month: "short",
                                                    year: "numeric",
                                                })
                            ...
                            <div className={`text-lg sm:text-xl font-bold sm:text-right font-mono self-start sm:self-center pl-13 sm:pl-0 ${item.type === "credit" ? "text-emerald-600" : "text-rose-600"
                                }`}>
