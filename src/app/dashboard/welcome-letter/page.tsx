...
// Using html2canvas-pro to handle modern CSS color functions natively
...
const WelcomeLetter = ({ userData }: WelcomeLetterProps) => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      // Scale 3 ensures high-quality text rendering for A4 printing
      const canvas = await html2canvas(element, { 
        scale: 3, 
        useCORS: true, 
        backgroundColor: "#ffffff", // Standard Hex for consistent rendering
        logging: false,
      });
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-12 min-h-screen px-4 w-full">
      {/* Action Bar */}
      <motion.div 
        initial={{ y: -20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-200 w-full max-w-2xl text-center sm:text-left"
      >
        <div className="flex-1">
          <p className="text-sm font-bold text-slate-900">Onboarding Document Ready</p>
          <p className="text-xs text-slate-500 font-medium">Download your official swadeshi welcome letter.</p>
        </div>
        <Button 
          onClick={downloadPDF} 
          disabled={isDownloading}
          className="bg-[#059669] hover:bg-[#047857] text-white gap-2 font-bold rounded-xl h-12 px-6 transition-all w-full sm:w-auto"
        >
          {isDownloading ? (
      ...
      {/* --- Responsive Wrapper for Mobile Viewing --- */}
      {/* This container scales the large A4 document to fit mobile screens visually */}
      <div className="w-full flex justify-center items-start overflow-hidden h-fit py-4">
        <div className="scale-[0.35] xs:scale-[0.45] sm:scale-[0.75] md:scale-100 origin-top transition-transform duration-500">
          <div 
            ref={letterRef} 
            style={{ 
            ...
            className="shadow-2xl p-[20mm] relative overflow-hidden border-[12px] border-[#ecfdf5] select-none"
          >
