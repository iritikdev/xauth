...
export function CartDrawer({ open, setOpen }: { open: boolean; setOpen: (o: boolean) => void }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen} >
      <SheetContent className="w-full sm:max-w-md flex flex-col bg-white border-none rounded-l-[3rem] p-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 z-[50]">
          {cart.items.length === 0 ? (
        ...
        {cart.items.length > 0 && (
          <div className="p-8 bg-slate-50 border-t border-slate-100 space-y-6">
            ...
            <Button className="w-full h-16 rounded-2xl bg-slate-900 hover:bg-emerald-600 text-white font-black uppercase tracking-widest text-xs transition-all">
