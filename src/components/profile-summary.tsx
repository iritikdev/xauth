...
import { 
  ...
  Phone, Mail, Fingerprint, Landmark
} from "lucide-react"
...
export default function UserProfileSummary({ userData, onEdit }: UserProfileSummaryProps) {
      ...
      <div ref={summaryRef} className="bg-white p-2 md:p-8 rounded-[3rem] border border-slate-100 shadow-sm">
        ...
        <header className="bg-[#0f172a] p-8 md:p-12 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl mb-16">
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="text-center md:text-left space-y-2">
              ...
              <h1 className="text-4xl md:text-5xl font-black tracking-tight">{userData?.name}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <DataSection 
            ...
            icon={Fingerprint} 
            items={[
              { label: "Father's Name", value: userData?.fatherName, icon: User },
              { label: "Email Address", value: userData?.email, icon: Mail },
              { label: "Mobile Number", value: userData?.mobile, icon: Phone },
              { label: "Aadhaar Card", value: `XXXX-XXXX-${userData?.aadharNo?.slice(-4)}`, isVerified: true },
              { label: "PAN Card", value: userData?.panNumber, isVerified: true },
            ]} 
          />
          ...
          <DataSection 
            ...
            items={[
              ...
              { label: "Branch Name", value: userData?.branch?.split('-')[1] || "Main Branch" },
            ]} 
          />
          ...
          <div className="md:col-span-2">
            <DataSection 
              ...
              items={[
                ...
                { label: "State & Pincode", value: `${userData?.state} - ${userData?.pincode}`, isVerified: true },
              ]} 
            />
          </div>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-slate-100 flex justify-center items-center gap-4 opacity-30">
            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Be Indian</span>
            <div className="h-1 w-1 bg-slate-900 rounded-full" />
            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Buy Indian</span>
            <div className="h-1 w-1 bg-slate-900 rounded-full" />
            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Grow Indian</span>
        </div>
      </div>
      {/* --- Capturable Area End --- */}
...
function DataSection({ title, icon: TitleIcon, items }: { title: string, icon: any, items: any[] }) {
  return (
